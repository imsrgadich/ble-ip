...
%sx = get_sigma_points(m,p,c,n);

% Progagate through the dynamic model; dt = t(i) - t(i-1)
dt = options.dt;
ukf.SX = options.A(dt) * sx; %% 4 x 4 * 4 x 9

% Prediction step: Get the predicted mean and covP_Fariance.
m_minus = sum(repmat(options.WM',n,1).* ukf.SX,2);
P_minus = zeros(n);

for i = 1:2*n+1
    P_minus = P_minus + options.WC(i) * (ukf.SX(:,i) - m_minus) * (ukf.SX(:,i) - m_minus)';
end
...
ukf.HX = [zeros(n,1) L -L];
ukf.HX = sqrt(n + options.lambda)*ukf.HX + repmat(m_minus,1,2*n+1);

%ukf.HX = get_sigma_points(m_minus,P_minus,c,n);

% Propagate the sigma points through the measurement model.

Y = gpPred(ukf.HX(1:2,:)',id-2,options);
...
for i = 1:2*n+1
    ...
    meas_cross_cov = meas_cross_cov + options.WC(i) * (ukf.SX(:,i) - m_minus) * (Y(i) - meas_mu)';
end
meas_cov = meas_cov + options.R;

% Compute the filter gain, estimated state and covariance
ukf.gain = meas_cross_cov / meas_cov; % 4 x 28
M = m_minus + ukf.gain * (y - meas_mu); % 4 x 1
P = P_minus - ukf.gain * meas_cov *  ukf.gain';
