function triton_run_code_all(job_id, ref_map_flag)

% TRITON_RUN_CODE to run the code with changing number of calibration
% points given through the file with job id.
...
% get the file numbers for calibration points and calibration time
params_file_num= read_params_all(job_id);

%% get the calibration points and time to be used for gp regression.
options.file_nums = read_params(params_file_num(1));
options.time_calib = read_params_1(params_file_num(2));
...
sprintf('saving the workspace to triton_files/workspace/calibration_points_time_old/%d.mat\n',job_id)
path_string = sprintf('/m/cs/work/gadichs1/ip/ble-ip-helvar/triton_files/workspace/calibration_points_time_old/%d.mat',job_id);
save(path_string)
