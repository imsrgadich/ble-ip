...
options.Q = 10^-1*eye(4);%+10^-2*[0 0 0.005 0;0 0 0 0.005; 0.005 0 0 0; 0 0.005 0 0];
...
% Process noise: only for location and angle measurement.              
options.Q = 10^-1*eye(options.N_states);
    ...
    case 3
        ...
        pause();
            ...
            for j = 2: size(testData,1)
                ...
                options.dt = tt(j) - tt(j-1);
                r = randtdn(options.numSamples,options.N_states);
...
% rmse_pf = rmse_1(m_true,m_F(:,1:2));
% 
% error = sqrt(sum((m_F(:,1:2)-m_true).^2,2));
% 
